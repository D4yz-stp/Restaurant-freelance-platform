...
try {
    $db = new PDO('sqlite:../../database/TesteOlga.db');
...
// Tratamento seguro para availability
$availabilityParams = [];
...
if ($experienceMin > 0) {
    $countQuery .= " AND fp.experience_years >= :exp_min";
}

if ($priceMin !== null) {
    $countQuery .= " AND s.base_price >= :price_min";
}

if ($priceMax !== null) {
    $countQuery .= " AND s.base_price <= :price_max";
}

if (!empty($availability) && !empty($availabilityConditions)) {
    $countQuery .= " AND (" . implode(" OR ", $availabilityConditions) . ")";
}

if (!empty($search)) {
    $countQuery .= " AND (s.title LIKE :search OR s.description LIKE :search OR sk.skill_name LIKE :search)";
}

// Adicionar limites para paginação à consulta principal
...
if ($experienceMin > 0) {
    $countStmt->bindParam(':exp_min', $experienceMin, PDO::PARAM_INT);
}

if ($priceMin !== null) {
    $countStmt->bindParam(':price_min', $priceMin, PDO::PARAM_STR);
}

if ($priceMax !== null) {
    $countStmt->bindParam(':price_max', $priceMax, PDO::PARAM_STR);
}

// Vincular parâmetros de disponibilidade
foreach ($availabilityParams as $paramName => $paramValue) {
    $countStmt->bindParam($paramName, $paramValue, PDO::PARAM_STR);
}
                    ...
                    <form action="" method="GET" id="filter-form">
                        ...
                        <div class="filter-section">
                            ...
                            <div class="price-inputs">
                                <input type="number" name="price_min" placeholder="Min" id="price-min" min="0" value="<?php echo $priceMin !== null ? $priceMin : ''; ?>">
                                <span>-</span>
                                <input type="number" name="price_max" placeholder="Max" id="price-max" min="0" value="<?php echo $priceMax !== null ? $priceMax : ''; ?>">
                            </div>
                        </div>
