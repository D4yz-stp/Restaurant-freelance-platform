...
$stmt = $db->prepare("
    SELECT U.first_name, U.last_name, U.email, U.contact, U.country, U.city, U.profile_image_url, R.role_name
    FROM Users U
...
if ($role === 'freelancer') {
    ...
    $stmt = $db->prepare("
        SELECT skill_id FROM FreelancerSkills 
        WHERE freelancer_id = (SELECT profile_id FROM FreelancerProfiles WHERE user_id = :id)
    ");
    $stmt->bindValue(':id', $user_id, SQLITE3_INTEGER);
