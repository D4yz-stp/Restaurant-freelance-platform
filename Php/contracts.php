...
$db_path = __DIR__ . '/../../ltw-project-ltw04g08/database/TesteOlga.db';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_contract']) && $role === 'restaurant') {
    ...
    if ($freelancer_id > 0 && $title && $start_date && $end_date) {
        $stmt = $db->prepare("
            INSERT INTO Contracts (
                ...
                (SELECT restaurant_id FROM RestaurantProfiles WHERE user_id = :restaurant_user_id),
                :freelancer_profile_id,
                :title, :description, :start_date, :end_date,
                :hourly_rate, :hours_per_week, 'pendente', CURRENT_TIMESTAMP
            )
        ");
        if ($stmt) {
            $stmt->bindValue(':restaurant_user_id', $user_id, SQLITE3_INTEGER);
            $stmt->bindValue(':freelancer_profile_id', $freelancer_id, SQLITE3_INTEGER);
...
if ($role === 'freelancer') {
    ...
    $freelancerQuery = $db->prepare("
        SELECT f.profile_id, u.first_name, u.last_name, f.hourly_rate, f.experience_years
        ...
        <?php if ($create_mode && $role === 'restaurant'): ?>
            <form method="POST" action="contracts.php" class="contract-form">
                    ...
                    <select id="freelancer_id" name="freelancer_id" required>
                        ...
                        <?php foreach ($freelancers as $freelancer): ?>
                            <option value="<?= htmlspecialchars($freelancer['profile_id']) ?>">
                                <?= htmlspecialchars($freelancer['first_name'] . ' ' . htmlspecialchars($freelancer['last_name'])) ?> 
                                (€<?= number_format($freelancer['hourly_rate'], 2) ?>/hora, <?= htmlspecialchars($freelancer['experience_years']) ?> anos exp.)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-actions">
                    <button type="submit" name="create_contract" class="btn btn-primary">Enviar Contrato</button>
                    <a href="contracts.php" class="btn btn-secondary">Cancelar</a>
                </div>
            </form>
        <?php else: ?>
